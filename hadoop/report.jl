...
# ╔═╡ c9865a8d-0de3-49ca-8eed-bf020ca6b38f
begin
	using Measurements
	using StatsPlots, DataFrames, CSV, Statistics
end

# ╔═╡ 9adee541-73b5-4416-a29e-b665b5034f03
md"""
...
# ╔═╡ d61ff790-69aa-4bc7-acb8-71691a807bd6
md"""
## Konfiguracja klastrów

Przeprowadziłem testy w następujących ustawieniach:
Oba testowane klasty korzystały z maszyn tego samego typu co pozwoli na 
łatwiejszą interpretację skalowalności systemu.
...
# ╔═╡ b398010c-dd3a-4c17-87db-b36d670524c9
md"""
Sekwencyjny program:
```rust
...
# ╔═╡ 26787c29-f805-4082-b29b-08a5df3ff865
md"""
Mapper:
```rust
extern crate efflux;
...
# ╔═╡ 7a95feaf-5f4c-48c4-8bd3-cb25fbd6c913
md"""
...
# ╔═╡ c23113ce-21e3-45a4-b096-c7800d686cf2
function parsetime(time)
	m, s = split(time, ['m', 's'])
	parse(Int,m) + parse(Float32, s) / 60
end

# ╔═╡ cb3c6a71-8653-45a4-869d-3076d5869a97
begin
	data = dropmissing(CSV.read("measurements.csv", DataFrame))
	data[!,:time] = parsetime.(data[!,:time])
	data = combine(groupby(data, [:size,:instances]), :time => (x-> measurement(mean(x), std(x))) => :time)
	i1 = data[data[:,:instances].==1,:]
	i2 = data[data[:,:instances].==2,:]
	i4 = data[data[:,:instances].==4,:]
end;

# ╔═╡ a9acaf26-cd63-4d87-a2b8-8c7d00bd1b8c
begin
    @df data plot(:size, :time, group=:instances, xlabel="Size [GB]", xticks=0:0.5:10, ylabel="time [min]")
    title!("Czasy dla poszczególnych konfiguracji.")
end

# ╔═╡ 71a42f66-099a-404f-8a40-3f5ce68ba228
begin
	@df i1 plot(:size, :time, label=:none, xlabel="Size [GB]", xticks=0:0.5:10, ylabel="time [min]")
	@df i1 scatter!(:size, :time, label=:none)
    title!("One node.")
end

# ╔═╡ 6cf56f69-b855-49bc-a6e1-86d11e3d803a
begin
	@df i2 plot(:size, :time, label=:none, xlabel="Size [GB]", xticks=0:0.5:10, ylabel="time [min]")
	@df i2 scatter!(:size, :time, label=:none)
    title!("Two nodes.")
end

# ╔═╡ b3cd2a0d-ba8e-4218-996d-9afab15662be
begin
	@df i4 plot(:size, :time, label=:none, xlabel="Size [GB]", xticks=0:0.5:10, ylabel="time [min]")
	@df i4 scatter!(:size, :time, label=:none)
    title!("Four nodes.")
end

# ╔═╡ 735cd8ae-cd09-490a-aa49-73d0cdc290cf
md"""
## Wykresy przyśpieszenia
"""


# ╔═╡ eeb220c4-3d5a-4702-a9c6-6f5dced0e43a
begin
    plot(i1[:,:size], i1[:,:time] ./ i2[:,:time], xlabel="Size [GB]", xticks=0:0.5:10, ylabel="speedup")
    title!("Przyśpieszenie dla 2 nodeów.")
end

# ╔═╡ 707e81f2-9a82-49dc-9987-0971b981783a
begin
    plot(i1[:,:size], i1[:,:time] ./ i4[:,:time], xlabel="Size [GB]", xticks=0:0.5:10, ylabel="speedup")
    title!("Przyśpieszenie dla 4 nodeów.")
end

# ╔═╡ f819b719-1730-419d-a7ea-49ed21d59ceb
md"""
## Podsumowanie

Widzimy, że sekwencyjna wersja programu okazła się być najszybsza. 
Nie jest to zaskakujące ze względu na ilość operacji IO w przypadku map-reducea.
Widzimy również iż mimo, że czas sekwencyjnego programu jest lepszy to map-reduce 
całkiem nieźle się skaluje wraz z dodawaniem kolejnych maszyn.

Dla rozmiaru danych testowanych tutaj 5Gb, 10Gb synchroniczna wersja programu osiągała 
znacznie lepsze wyniki ale dla 100x większych danych konieczne byłoby wykorzystanie 
systemu takiego jak map-reduce.

Wygląda na to, że COST w naszym przypadku to 16 nodeów (zakładając brak dużego wzrostu
czasu wykonywania ze względu na IO).
