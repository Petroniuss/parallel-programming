...
long int compute_points_within(long int n) {
...
/*
    Useful mpi functions:
        ...
        - Reduce - sums all the data. (we need master which will hold the reduced data)
*/
...
int main(int argc, char* argv[]) {
    ...
    long long int local_count = compute_points_within(local_n_points);
    long long int total_count;
    MPI_Reduce(&local_count, &total_count, 1, MPI_LONG_LONG_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    if (rank == 0) {
        double pi = (((double) total_count) / n_points) * 4;
        double time = MPI_Wtime() - start_time;
        printf("%d,%.*f,%.*f,%lld,%lld\n", n_nodes, DBL_DECIMAL_DIG, pi, DBL_DECIMAL_DIG, time, n_points, total_count);
    }
